...
export class AppConfigService {
	...
	public getBoolean(key: string): boolean {
		const keyString = this.get(key);
		return Boolean(keyString === 'true');
	}

	get frontEndUrl(): string {
		return this.get('FRONT_END_URL') || '';
	}
	...
	public getRmqOptions(queue: string, serivceName?: string) {
	...
	// public getKafkaOptions(groupKey: string, clientKey?: string): KafkaOptions {
	// 	const brokers = ['KAFKA_URI_1', 'KAFKA_URI_2', 'KAFKA_URI_3']
	// 		.map((_key) => {
	...
	// 	const groupId = this.get(groupKey);
	// 	const clientId = this.get(clientKey);
